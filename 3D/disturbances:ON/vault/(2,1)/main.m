function main
  ...
  % Penalty matrices
%   rn             = 0.3 * rand(3);
%   Q              = 0.1 * (eye(3) + [0 0 0; 0 0 0; 0 0 -0.5] + rn);
%   R              = 0.01 * eye(2);
%   P              = 0.1 * (eye(3) + rn);
  
  Q              = 0.5*[0.7349    0.0506    0.0034;
  ...
  dist_denom     = L_v / L_g * exp(L_g * (N * T - T)) * (exp(L_g * T) - 1)
  epsilon_psi    = disturbance * dist_denom + epsilon_omega
...
%   omega_psi      = disturbance / (3 * L_g) * exp(L_g * (N * T - T)) * (exp(L_g * T) - 1) + sqrt(D)/2 + ptol
%
...
%   epsilon_psi    = 3 * svds(P,1) * omega_psi^2
  ...
  for k = 1:total_iterations
    ...
    deactivate_constraints = agent_1_arrived_at_terminal_region && agent_2_arrived_at_terminal_region;
