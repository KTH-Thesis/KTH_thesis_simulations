function [t_3, x_3, u_3, x_open_loop_3, u_open_loop_3] = nmpc_3(runningcosts, terminalcosts, ...
...
%                         sampling instant.
%          Usage: [cost] = runningcosts(t_3, x_3, u_3)
%                 with time t_3, state x_3 and control u_3
...
%          Usage: cost = terminalcosts(t_3, x_3)
%                 with time t_3 and state x_3
%   constraints:          computes the value of the restrictions for a
%                         sampling instance provided the data t_3, x_3 and u_3
...
%                         control.
%          Usage: [A, b, Aeq, beq, lb, ub] = linearconstraints(t_3, x_3, u_3)
%                 with time t_3, state x_3 and control u_3
%   system:               evaluates the difference equation describing the
%                         process given time t_3, state vector x_3 and control
%                         u_3.
%                         The function returns the state vector x_3 at the
%                         next time instant.
%          Usage: [y] = system(t_3, x_3, u_3, T)
%                 with time t_3, state x_3, control u_3 and sampling interval T
...
%   applyControl:                 applies the first control element of u_3 to
...
%                                 control u_3, the corresponding value of the
...
%                                 data t0, x0 and u_3 given by the
%                                 optimization method.
%                                 The function returns the value of the
...
%                                 interval [t0, tf] using the control u_3.
%                                 The function returns the state vector x_3
...
% (C) Lars Gruene, Juergen Pannek 2011
    ...
    while(mpciter < mpciterations)
        ...
        x_3_predicted = computeOpenloopSolution(system, N, T, t0, x0, u_new, ...
                                    atol_ode_sim, rtol_ode_sim, type);

        x_open_loop_3 = x_3_predicted;
...
function [tapplied, xapplied] = applyControl(system, T, t0, x0, u_3, ...
                                atol_ode_real, rtol_ode_real, type)
    xapplied = dynamic(system, T, t0, x0, u_3(:,1), ...
                       atol_ode_real, rtol_ode_real, type);
    tapplied = t0+T;
end

function u0 = shiftHorizon(u_3)
    u0 = [u_3(:,2:size(u_3,2)) u_3(:,size(u_3,2))];
end

function [u_3, V, exitflag, output] = solveOptimalControlProblem ...
    (runningcosts, terminalcosts, constraints, terminalconstraints, ...
    linearconstraints, system, N, t0, x0, u0, T, ...
    atol_ode_sim, rtol_ode_sim, tol_opt, options, type)

    x_3 = zeros(N+1, length(x0));
    x_3 = computeOpenloopSolution(system, N, T, t0, x0, u0, ...
                                atol_ode_sim, rtol_ode_sim, type);
    ...
    for k=1:N
        [Anew, bnew, Aeqnew, beqnew, lbnew, ubnew] = ...
               linearconstraints(t0+k*T,x_3(k,:),u0(:,k));
        A = blkdiag(A,Anew);
        b = [b, bnew];
    ...
    [u_3, V, exitflag, output] = fmincon(@(u_3) costfunction(runningcosts, ...
        terminalcosts, system, N, T, t0, x0, ...
        u_3, atol_ode_sim, rtol_ode_sim, type), u0, A, b, Aeq, beq, lb, ...
        ub, @(u_3) nonlinearconstraints(constraints, terminalconstraints, ...
        system, N, T, t0, x0, u_3, ...
...
function cost = costfunction(runningcosts, terminalcosts, system, ...
    ...
    cost = 0;
    x_3 = zeros(N+1, length(x0));
    x_3 = computeOpenloopSolution(system, N, T, t0, x0, u_3, ...
                                atol_ode_sim, rtol_ode_sim, type);
    for k=1:N
        cost = cost+runningcosts(t0+k*T, x_3(k,:), u_3(:,k));
    end
    cost = cost+terminalcosts(t0+(N+1)*T, x_3(N+1,:));
end

function [c,ceq] = nonlinearconstraints(constraints, ...
    terminalconstraints, system, ...
    N, T, t0, x0, u_3, atol_ode_sim, rtol_ode_sim, type)
    x_3 = zeros(N+1, length(x0));
    x_3 = computeOpenloopSolution(system, N, T, t0, x0, u_3, ...
                                atol_ode_sim, rtol_ode_sim, type);
    c = [];
    ceq = [];
    for k=1:N
        [cnew, ceqnew] = constraints(t0+k*T,x_3(k,:),u_3(:,k));
        c = [c cnew];
        ceq = [ceq ceqnew];
    end
    [cnew, ceqnew] = terminalconstraints(t0+(N+1)*T,x_3(N+1,:));
    c = [c cnew];
    ceq = [ceq ceqnew];
end

function x_3 = computeOpenloopSolution(system, N, T, t0, x0, u_3, ...
                                     atol_ode_sim, rtol_ode_sim, type)
    x_3(1,:) = x0;
    for k=1:N
        x_3(k+1,:) = dynamic(system, T, t0, x_3(k,:), u_3(:,k), ...
                             atol_ode_sim, rtol_ode_sim, type);
    end
end

function [x_3, t_intermediate, x_intermediate] = dynamic(system, T, t0, ...
             x0, u_3, atol_ode, rtol_ode, type)
    if ( strcmp(type, 'difference equation') )
        x_3 = system(t0, x0, u_3, T);
        x_intermediate = [x0; x_3];
    ...
    elseif ( strcmp(type, 'differential equation') )
        ...
        [t_intermediate,x_intermediate] = ode45(system, ...
            [t0, t0+T], x0, options, u_3);
        x_3 = x_intermediate(size(x_intermediate,1),:);
    end
end

function printSolution(system, printHeader, printClosedloopData, ...
             plotTrajectories, mpciter, T, t0, x0, u_3, ...
             atol_ode, rtol_ode, type, iprint, exitflag, output, t_Elapsed)
    if (mpciter == 0)
        printHeader();
    end
    printClosedloopData(mpciter, u_3, x0, t_Elapsed);
    switch exitflag
        case -2
    ...
    if ( iprint >= 5 )
        plotTrajectories(@dynamic, system, T, t0, x0, u_3, atol_ode, rtol_ode, type)
    end
end
