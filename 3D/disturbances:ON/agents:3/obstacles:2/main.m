...
%   P              = 0.5 * (eye(3) + rn);
  ...
  Q              = 0.8*[0.7349    0.000506    0.0034;
                    0.01260    0.7349    0.0710;
                    0.02113    0.01336    0.7349];
  P              = 0.8*Q;
...
function [c,ceq] = constraints_1(t_1, e_1, u_1)
  ...
  if deactivate_constraints == false
    ...
    for i = 1:2
      c(i) = (obs(i,3) + r(1) + otol)^2 - ((x+des_1(1) - obs(i,1))^2 + (y+des_1(2) - obs(i,2))^2);
    end
    ...
    for i = 1:2
      c(4+i) = (obs(i,3) + r(1) + otol)^2 - ((x+des_1(1) - obs(i,1))^2 + (y+des_1(2) - obs(i,2))^2);
    end
...
function [c,ceq] = constraints_3(t_3, e_3, u_3)
  ...
  if deactivate_constraints == false
    ...
    % Avoid collision with obstacles -- max
    for i = 1:2
      c(i) = (obs(i,3) + r(2) + otol) - sqrt((x+des_3(1) - obs(i,1))^2 + (y+des_3(2) - obs(i,2))^2);
    end
    ...
    % Avoid collision with obstacles -- max
    for i = 1:2
      c(4+i) = (obs(i,3) + r(2) + otol) - sqrt((x+des_3(1) - obs(i,1))^2 + (y+des_3(2) - obs(i,2))^2);
    end
