...
%   L_g            = u_max * sqrt(sum(sum(Q(1:2,1:2))))
%   
%   D              = 4*epsilon_omega / (3 * svds(P,1));  
%   omega_psi_1    = disturbance / (3 * L_g) * exp(L_g * (N * T - T)) * (exp(L_g * T) - 1) + sqrt(D)/2;
%   omega_psi_2    = disturbance / (3 * L_g) * exp(L_g * (N * T - T)) * (exp(L_g * T) - 1) - sqrt(D)/2;
%   
%   omega_psi = 0;
%   if omega_psi_1 >= omega_psi_2
%     omega_psi = omega_psi_1
%   else
%     omega_psi = omega_psi_2
%   end
%     
