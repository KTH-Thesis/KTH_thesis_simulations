function [t_2, x_2, u_2, x_open_loop_2, u_open_loop_2] = nmpc_2(runningcosts, terminalcosts, ...
              constraints, terminalconstraints, ...
              linearconstraints, system, real_system, ...
...
%                         sampling instant.
%          Usage: [cost] = runningcosts(t_2, x_2, u_2)
%                 with time t_2, state x_2 and control u_2
...
%          Usage: cost = terminalcosts(t_2, x_2)
%                 with time t_2 and state x_2
%   constraints:          computes the value of the restrictions for a
%                         sampling instance provided the data t_2, x_2 and u_2
...
%                         control.
%          Usage: [A, b, Aeq, beq, lb, ub] = linearconstraints(t_2, x_2, u_2)
%                 with time t_2, state x_2 and control u_2
%   system:               evaluates the difference equation describing the
%                         process given time t_2, state vector x_2 and control
%                         u_2.
%                         The function returns the state vector x_2 at the
%                         next time instant.
%          Usage: [y] = system(t_2, x_2, u_2, T)
%                 with time t_2, state x_2, control u_2 and sampling interval T
...
%   applyControl:                 applies the first control element of u_2 to
...
%                                 control u_2, the corresponding value of the
...
%                                 data t0, x0 and u_2 given by the
%                                 optimization method.
%                                 The function returns the value of the
...
%                                 interval [t0, tf] using the control u_2.
%                                 The function returns the state vector x_2
...
% (C) Lars Gruene, Juergen Pannek 2011
    ...
    while(mpciter < mpciterations)
        ...
        % Acquire the predicted state
        x_2_predicted = computeOpenloopSolution(system, N, T, t0, x0, u_new, ...
                                    atol_ode_sim, rtol_ode_sim, type);

        global x_open_loop_2;
        x_open_loop_2 = x_2_predicted;
        ...
        [tmeasure, xmeasure] = applyControl(real_system, T, t0, x0, u_new, ...
...
function [tapplied, xapplied] = applyControl(system, T, t0, x0, u_2, ...
                                atol_ode_real, rtol_ode_real, type)
    xapplied = dynamic(system, T, t0, x0, u_2(:,1), ...
                       atol_ode_real, rtol_ode_real, type);
    tapplied = t0+T;
end

function u0 = shiftHorizon(u_2)
    u0 = [u_2(:,2:size(u_2,2)) u_2(:,size(u_2,2))];
end

function [u_2, V, exitflag, output] = solveOptimalControlProblem ...
    (runningcosts, terminalcosts, constraints, terminalconstraints, ...
    linearconstraints, system, N, t0, x0, u0, T, ...
    atol_ode_sim, rtol_ode_sim, tol_opt, options, type)

    x_2 = zeros(N+1, length(x0));
    x_2 = computeOpenloopSolution(system, N, T, t0, x0, u0, ...
                                atol_ode_sim, rtol_ode_sim, type);
    ...
    for k=1:N
        [Anew, bnew, Aeqnew, beqnew, lbnew, ubnew] = ...
               linearconstraints(t0+k*T,x_2(k,:),u0(:,k));
        A = blkdiag(A,Anew);
        b = [b, bnew];
    ...
    [u_2, V, exitflag, output] = fmincon(@(u_2) costfunction(runningcosts, ...
        terminalcosts, system, N, T, t0, x0, ...
        u_2, atol_ode_sim, rtol_ode_sim, type), u0, A, b, Aeq, beq, lb, ...
        ub, @(u_2) nonlinearconstraints(constraints, terminalconstraints, ...
        system, N, T, t0, x0, u_2, ...
...
function cost = costfunction(runningcosts, terminalcosts, system, ...
    ...
    cost = 0;
    x_2 = zeros(N+1, length(x0));
    x_2 = computeOpenloopSolution(system, N, T, t0, x0, u_2, ...
                                atol_ode_sim, rtol_ode_sim, type);
    for k=1:N
        cost = cost+runningcosts(t0+k*T, x_2(k,:), u_2(:,k));
    end
    cost = cost+terminalcosts(t0+(N+1)*T, x_2(N+1,:));
end

function [c,ceq] = nonlinearconstraints(constraints, ...
    terminalconstraints, system, ...
    N, T, t0, x0, u_2, atol_ode_sim, rtol_ode_sim, type)
    x_2 = zeros(N+1, length(x0));
    x_2 = computeOpenloopSolution(system, N, T, t0, x0, u_2, ...
                                atol_ode_sim, rtol_ode_sim, type);
    c = [];
    ceq = [];
    for k=1:N
        [cnew, ceqnew] = constraints(t0+k*T,x_2(k,:),u_2(:,k));
        c = [c cnew];
        ceq = [ceq ceqnew];
    end
    [cnew, ceqnew] = terminalconstraints(t0+(N+1)*T,x_2(N+1,:));
    c = [c cnew];
    ceq = [ceq ceqnew];
end

function x_2 = computeOpenloopSolution(system, N, T, t0, x0, u_2, ...
                                     atol_ode_sim, rtol_ode_sim, type)
    x_2(1,:) = x0;
    for k=1:N
        x_2(k+1,:) = dynamic(system, T, t0, x_2(k,:), u_2(:,k), ...
                             atol_ode_sim, rtol_ode_sim, type);
    end
end

function [x_2, t_intermediate, x_intermediate] = dynamic(system, T, t0, ...
             x0, u_2, atol_ode, rtol_ode, type)
    if ( strcmp(type, 'difference equation') )
        x_2 = system(t0, x0, u_2, T);
        x_intermediate = [x0; x_2];
    ...
    elseif ( strcmp(type, 'differential equation') )
        ...
        [t_intermediate,x_intermediate] = ode45(system, ...
            [t0, t0+T], x0, options, u_2);
        x_2 = x_intermediate(size(x_intermediate,1),:);
    end
end

function printSolution(system, printHeader, printClosedloopData, ...
             plotTrajectories, mpciter, T, t0, x0, u_2, ...
             atol_ode, rtol_ode, type, iprint, exitflag, output, t_Elapsed)
    if (mpciter == 0)
        printHeader();
    end
    printClosedloopData(mpciter, u_2, x0, t_Elapsed);
    switch exitflag
        case -2
    ...
    if ( iprint >= 5 )
        plotTrajectories(@dynamic, system, T, t0, x0, u_2, atol_ode, rtol_ode, type)
    end
end
