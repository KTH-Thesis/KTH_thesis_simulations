function [t_1, x_1, u_1, x_open_loop_1, u_open_loop_1] = nmpc_1(runningcosts, terminalcosts, ...
...
%                         sampling instant.
%          Usage: [cost] = runningcosts(t_1, x_1, u_1)
%                 with time t_1, state x_1 and control u_1
...
%          Usage: cost = terminalcosts(t_1, x_1)
%                 with time t_1 and state x_1
%   constraints:          computes the value of the restrictions for a
%                         sampling instance provided the data t_1, x_1 and u_1
...
%                         control.
%          Usage: [A, b, Aeq, beq, lb, ub] = linearconstraints(t_1, x_1, u_1)
%                 with time t_1, state x_1 and control u_1
%   system:               evaluates the difference equation describing the
%                         process given time t_1, state vector x_1 and control
%                         u_1.
%                         The function returns the state vector x_1 at the
%                         next time instant.
%          Usage: [y] = system(t_1, x_1, u_1, T)
%                 with time t_1, state x_1, control u_1 and sampling interval T
...
%   applyControl:                 applies the first control element of u_1 to
...
%                                 control u_1, the corresponding value of the
...
%                                 data t0, x0 and u_1 given by the
%                                 optimization method.
%                                 The function returns the value of the
...
%                                 interval [t0, tf] using the control u_1.
%                                 The function returns the state vector x_1
...
function [tapplied, xapplied] = applyControl(system, T, t0, x0, u_1, ...
                                atol_ode_real, rtol_ode_real, type)
    xapplied = dynamic(system, T, t0, x0, u_1(:,1), ...
                       atol_ode_real, rtol_ode_real, type);
    tapplied = t0+T;
end

function u0 = shiftHorizon(u_1)
    u0 = [u_1(:,2:size(u_1,2)) u_1(:,size(u_1,2))];
end

function [u_1, V, exitflag, output] = solveOptimalControlProblem ...
    (runningcosts, terminalcosts, constraints, terminalconstraints, ...
    linearconstraints, system, N, t0, x0, u0, T, ...
    atol_ode_sim, rtol_ode_sim, tol_opt, options, type)

    x_1 = zeros(N+1, length(x0));
    x_1 = computeOpenloopSolution(system, N, T, t0, x0, u0, ...
                                atol_ode_sim, rtol_ode_sim, type);
    ...
    for k=1:N
        [Anew, bnew, Aeqnew, beqnew, lbnew, ubnew] = ...
               linearconstraints(t0+k*T,x_1(k,:),u0(:,k));
        A = blkdiag(A,Anew);
        b = [b, bnew];
    ...
    [u_1, V, exitflag, output] = fmincon(@(u_1) costfunction(runningcosts, ...
        terminalcosts, system, N, T, t0, x0, ...
        u_1, atol_ode_sim, rtol_ode_sim, type), u0, A, b, Aeq, beq, lb, ...
        ub, @(u_1) nonlinearconstraints(constraints, terminalconstraints, ...
        system, N, T, t0, x0, u_1, ...
...
function cost = costfunction(runningcosts, terminalcosts, system, ...
    ...
    cost = 0;
    x_1 = zeros(N+1, length(x0));
    x_1 = computeOpenloopSolution(system, N, T, t0, x0, u_1, ...
                                atol_ode_sim, rtol_ode_sim, type);
    for k=1:N
        cost = cost+runningcosts(t0+k*T, x_1(k,:), u_1(:,k));
    end
    cost = cost+terminalcosts(t0+(N+1)*T, x_1(N+1,:));
end

function [c,ceq] = nonlinearconstraints(constraints, ...
    terminalconstraints, system, ...
    N, T, t0, x0, u_1, atol_ode_sim, rtol_ode_sim, type)
    x_1 = zeros(N+1, length(x0));
    x_1 = computeOpenloopSolution(system, N, T, t0, x0, u_1, ...
                                atol_ode_sim, rtol_ode_sim, type);
    c = [];
    ceq = [];
    for k=1:N
        [cnew, ceqnew] = constraints(t0+k*T,x_1(k,:),u_1(:,k));
        c = [c cnew];
        ceq = [ceq ceqnew];
    end
    [cnew, ceqnew] = terminalconstraints(t0+(N+1)*T,x_1(N+1,:));
    c = [c cnew];
    ceq = [ceq ceqnew];
end

function x_1 = computeOpenloopSolution(system, N, T, t0, x0, u_1, ...
                                     atol_ode_sim, rtol_ode_sim, type)
    x_1(1,:) = x0;
    for k=1:N
        x_1(k+1,:) = dynamic(system, T, t0, x_1(k,:), u_1(:,k), ...
                             atol_ode_sim, rtol_ode_sim, type);
    end
end

function [x_1, t_intermediate, x_intermediate] = dynamic(system, T, t0, ...
             x0, u_1, atol_ode, rtol_ode, type)
    if ( strcmp(type, 'difference equation') )
        x_1 = system(t0, x0, u_1, T);
        x_intermediate = [x0; x_1];
    ...
    elseif ( strcmp(type, 'differential equation') )
        ...
        [t_intermediate,x_intermediate] = ode45(system, ...
            [t0, t0+T], x0, options, u_1);
        x_1 = x_intermediate(size(x_intermediate,1),:);
    end
end

function printSolution(system, printHeader, printClosedloopData, ...
             plotTrajectories, mpciter, T, t0, x0, u_1, ...
             atol_ode, rtol_ode, type, iprint, exitflag, output, t_Elapsed)
    if (mpciter == 0)
        printHeader();
    end
    printClosedloopData(mpciter, u_1, x0, t_Elapsed);
    switch exitflag
        case -2
    ...
    if ( iprint >= 5 )
        plotTrajectories(@dynamic, system, T, t0, x0, u_1, atol_ode, rtol_ode, type)
    end
end
